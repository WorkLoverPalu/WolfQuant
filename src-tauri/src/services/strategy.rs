...
use crate::models::{InvestmentStrategy, StrategyApplication, BacktestResult, PerformancePoint, Transaction};
use chrono::Utc;
use log::{error, info};
use rusqlite::params;
...
pub fn create_investment_strategy(
    ...
    if let Err(e) = serde_json::from_str::<Value>(parameters) {
        return Err(AuthError::InvalidCredentials(format!("无效的策略参数: {}", e)));
    }
    ...
    info!("Investment strategy created: {} for user: {}", name, user_id);
    Ok(strategy)
}
...
pub fn update_investment_strategy(
    ...
    info!("Investment strategy updated: {} for user: {}", name, user_id);
    Ok(strategy)
}
...
pub fn get_user_investment_strategies(user_id: &str) -> Result<Vec<InvestmentStrategy>, AuthError> {
    ...
    let strategies = stmt.query_map(params![user_id], |row| {
...
pub fn apply_strategy(
    ...
    info!("Strategy applied: {} to asset: {} for user: {}", strategy_name, asset_name, user_id);
    Ok(application)
}
...
pub fn remove_strategy_application(id: i64, user_id: &str) -> Result<(), AuthError> {
    ...
    if !application_exists {
        return Err(AuthError::InvalidCredentials("策略应用不存在或无权限".to_string()));
    }
...
pub fn get_user_strategy_applications(user_id: &str, asset_id: Option<i64>) -> Result<Vec<StrategyApplication>, AuthError> {
    let conn = get_db_connection()?;
...
pub fn backtest_strategy(
    ...
    let price_data = stmt.query_map(params![asset_id, start_date, end_date], |row| {
    ...
    if price_data.is_empty() {
        return Err(AuthError::InvalidCredentials("所选时间范围内没有历史价格数据".to_string()));
    }
    ...
    let backtest_result = match strategy_type.as_str() {
        ...
        _ => return Err(AuthError::InvalidCredentials("不支持的策略类型".to_string())),
    };
    
    info!("Strategy backtest completed for strategy: {} on asset: {}", strategy_id, asset_id);
    Ok(backtest_result)
}
...
fn backtest_macd_strategy(
    ...
    if price_data.len() < slow_period + signal_period {
        return Err(AuthError::InvalidCredentials("历史数据不足，无法执行MACD策略回测".to_string()));
    }
    ...
    // 计算MACD
    let (macd_line, signal_line, _) = calculate_macd(&prices, fast_period, slow_period, signal_period);
    ...
    for i in (slow_period + signal_period)..prices.len() {
        ...
        let benchmark_value = initial_investment * (current_price / prices[slow_period + signal_period]);
...
fn backtest_rsi_strategy(
    ...
    if price_data.len() < period + 1 {
        return Err(AuthError::InvalidCredentials("历史数据不足，无法执行RSI策略回测".to_string()));
    }
...
fn backtest_ma_strategy(
    ...
    if price_data.len() < long_period {
        return Err(AuthError::InvalidCredentials("历史数据不足，无法执行均线策略回测".to_string()));
    }
    ...
    for i in long_period..prices.len() {
        ...
        // 短期均线下穿长期均线（死叉，卖出信号）
        else if prev_short_ma_value > prev_long_ma_value && short_ma_value < long_ma_value && shares > 0.0 {
            let value = shares * current_price;
            cash += value;
...
fn backtest_custom_strategy(
    ...
    if price_data.len() < 2 {
        return Err(AuthError::InvalidCredentials("历史数据不足，无法执行自定义策略回测".to_string()));
    }
    ...
    for i in 1..prices.len() {
        ...
        // 价格上涨超过阈值（卖出信号）
        else if last_buy_price > 0.0 && (current_price - last_buy_price) / last_buy_price >= sell_threshold && shares > 0.0 {
            let value = shares * current_price;
            cash += value;
