...
/// 回测引擎（Backtester）
///
/// 该模块实现了一个基本的量化回测引擎，支持策略回测、绩效评估和权益曲线记录。
///
/// # 主要结构体
///
/// - `BacktestConfig`：回测参数配置，包括初始资金、手续费率和滑点。
/// - `BacktestResult`：回测结果，包含交易记录、绩效指标和权益曲线。
/// - `PerformanceMetrics`：绩效指标，包括总收益率、年化收益率、夏普比率、最大回撤、胜率、盈亏比等。
/// - `EquityPoint`：权益曲线的单个点，包含时间戳和权益值。
/// - `Backtester`：回测引擎主体，负责驱动策略、处理信号、管理投资组合和计算绩效。
///
/// # 主要方法
///
/// - `Backtester::new`：创建新的回测引擎实例。
/// - `Backtester::run`：运行回测流程，返回回测结果。
/// - `Backtester::process_signal`：处理策略产生的交易信号，应用滑点并生成订单。
/// - `Backtester::calculate_performance`：根据回测过程中的数据计算各项绩效指标。
///
/// # 用法示例
///
/// ```rust
/// let config = BacktestConfig { initial_capital: 100000.0, fee_rate: 0.001, slippage: 0.0005 };
/// let strategy: Box<dyn Strategy> = ...;
/// let candles: Vec<Candle> = ...;
/// let mut backtester = Backtester::new(config, strategy, candles);
...
/// # 注意事项
///
/// - 策略需实现 `Strategy` trait。
/// - 投资组合管理和订单处理依赖于 `Portfolio` 和 `Order` 相关实现。
/// - 性能指标计算采用简化模型，实际应用可根据需求扩展。
/// 
