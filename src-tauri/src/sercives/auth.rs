...
use crate::database::{get_db_connection, execute_query};
use crate::error::AuthError;
use crate::models::{PasswordResetToken, User};
use crate::utils::crypto::{hash_password, verify_password, generate_token};
...
pub fn login_user(username_or_email: &str, password: &str) -> Result<(User, String), AuthError> {
    ...
    match result {
        ...
        Err(_) => Err(AuthError::InvalidCredentials("用户名或邮箱不存在".to_string())),
    }
}
...
pub fn create_password_reset_token(email: &str) -> Result<PasswordResetToken, AuthError> {
    ...
    let token = generate_token();
    let config = Config::get();
    let expiry = Utc::now() + Duration::minutes(config.auth.password_reset_token_expiry_minutes as i64);
