...
pub async fn get_candles(
    ...
    // 如果数据库中没有，则从适配器获取
    engine
        .import_historical_data(&symbol, &asset_type, &source, start, end, &interval)
        .await
}
...
pub async fn start_market_data(
    ...
    engine
        .start_market_data(&symbol, &asset_type, &source)
        .await
}

#[command]
pub async fn stop_market_data(engine: State<'_, Engine>) -> Result<(), String> {
    engine.stop();
    Ok(())
}
